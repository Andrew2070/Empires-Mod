...
public class FlagsConfig extends JsonConfig<FlagsConfig.Wrapper, List<FlagsConfig.Wrapper>> {
	public FlagsConfig(String path) {
		...
		gsonType = new TypeToken<List<Wrapper>>() {
		}.getType();
	...
	public boolean validate(List<Wrapper> items) {
		...
		for (Iterator<Wrapper> it = items.iterator(); it.hasNext();) {
			...
			if (!item.getFlagType().type.isAssignableFrom(item.defaultState.getClass())) {
				Empires.instance.LOG
						.error("The default value for the flag is of invalid type for flag " + item.flagName.toString()
								+ "! Needed " + item.getFlagType().type.getSimpleName() + " Removing...");
				it.remove();
				isValid = false;
			}
		}
		...
		for (FlagType type : FlagType.values()) {
			...
			if (!ok) {
				items.add(new Wrapper(type.name, type.defaultValue, type.configurable));
				Empires.instance.LOG.error(
						"Flag config is missing (or is an invalid entry) {} flag! Adding with default settings...",
						type.name);
				isValid = false;
			}
		}
