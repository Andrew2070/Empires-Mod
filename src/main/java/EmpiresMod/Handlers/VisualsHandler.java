...
public class VisualsHandler {
	...
	public void tick(TickEvent.ServerTickEvent ev) {
		...
		if (!markedBlocks.isEmpty()) {
			for (Iterator<VisualObject> visualObjectIterator = markedBlocks.iterator(); visualObjectIterator
					.hasNext();) {
				VisualObject visualObject = visualObjectIterator.next();

				for (Iterator<BlockCoords> blockCoordsIterator = visualObject.blockCoords
	...
	public void markCorners(int selectionX1, int selectionY1, int selectionZ1, int selectionX2, int selectionY2,
			int selectionZ2, int dim, EntityPlayerMP caller) {
		...
		// On the X
		blockList.add(new BlockCoords(selectionX1 + (selectionX1 > selectionX2 ? -1 : 1), selectionY1, selectionZ1, dim,
				Blocks.redstone_block));
		blockList.add(new BlockCoords(selectionX2 + (selectionX1 > selectionX2 ? 1 : -1), selectionY2, selectionZ2, dim,
				Blocks.redstone_block));
		blockList.add(new BlockCoords(selectionX1 + (selectionX1 > selectionX2 ? -2 : 2), selectionY1, selectionZ1, dim,
				Blocks.redstone_block));
		blockList.add(new BlockCoords(selectionX2 + (selectionX1 > selectionX2 ? 2 : -2), selectionY2, selectionZ2, dim,
				Blocks.redstone_block));

		// On the Z
		blockList.add(new BlockCoords(selectionX2, selectionY2, selectionZ2 + (selectionZ1 > selectionZ2 ? 1 : -1), dim,
				Blocks.redstone_block));
		blockList.add(new BlockCoords(selectionX1, selectionY1, selectionZ1 + (selectionZ1 > selectionZ2 ? -1 : 1), dim,
				Blocks.redstone_block));
		blockList.add(new BlockCoords(selectionX2, selectionY2, selectionZ2 + (selectionZ1 > selectionZ2 ? 2 : -2), dim,
				Blocks.redstone_block));
		blockList.add(new BlockCoords(selectionX1, selectionY1, selectionZ1 + (selectionZ1 > selectionZ2 ? -2 : 2), dim,
				Blocks.redstone_block));

		if (selectionY1 != selectionY2) {
			// On the Y
			blockList.add(new BlockCoords(selectionX1, selectionY1 + (selectionY1 > selectionY2 ? -1 : 1), selectionZ1,
					dim, Blocks.redstone_block));
			blockList.add(new BlockCoords(selectionX2, selectionY2 + (selectionY1 > selectionY2 ? 1 : -1), selectionZ2,
					dim, Blocks.redstone_block));
			blockList.add(new BlockCoords(selectionX1, selectionY1 + (selectionY1 > selectionY2 ? -2 : 2), selectionZ1,
					dim, Blocks.redstone_block));
			blockList.add(new BlockCoords(selectionX2, selectionY2 + (selectionY1 > selectionY2 ? 2 : -2), selectionZ2,
					dim, Blocks.redstone_block));
		}
	...
	public void markPlotBorders(Plot plot, EntityPlayerMP caller) {
		markPlotBorders(plot.getStartX(), plot.getStartY(), plot.getStartZ(), plot.getEndX(), plot.getEndY(),
				plot.getEndZ(), plot.getDim(), caller, plot);
	}
	...
	public void markEmpireBorders(EmpireBlock.Container empireBlocksContainer, EntityPlayerMP caller) {
		...
		for (EmpireBlock block : empireBlocksContainer.values()) {

			// Showing lines in borders
			for (int i = 0; i < 8; i += 2) {
				if (empireBlocksContainer.get(block.getDim(), block.getX() + dx[i], block.getZ() + dz[i]) == null) {
					if (dx[i] == 0) {
		...
		addMarkedBlocks(caller, empireBlocksContainer, blockList);
	}
	...
	public void updateEmpireBorders(EmpireBlock.Container empireBlocksContainer) {
		List<EntityPlayerMP> callers = new ArrayList<EntityPlayerMP>();
		for (VisualObject visualObject : markedBlocks) {
			if (visualObject.isEmpire() && visualObject.object.equals(empireBlocksContainer)) {
				for (BlockCoords coords : visualObject.blockCoords) {
					coords.deleted = true;
				}
				callers.add(visualObject.player);
		...
		for (EntityPlayerMP player : callers) {
			markEmpireBorders(empireBlocksContainer, player);
		}
	}
	...
	public void unmarkEmpires(EntityPlayerMP caller) {
		for (VisualObject visualObject : markedBlocks) {
			if (visualObject.player == caller && visualObject.isEmpire()) {
				for (BlockCoords blockCoords : visualObject.blockCoords)
					blockCoords.deleted = true;
			}
		}
	...
	private class VisualObject {
		...
		public boolean isEmpire() {
			return object != null && object instanceof EmpireBlock.Container;
		}
