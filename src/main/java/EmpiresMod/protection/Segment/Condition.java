...
import EmpiresMod.exceptions.Empires.ConditionException;
...
public class Condition {
	...
	public boolean execute(Object object, Getter.Container getters) throws ConditionException, GetterException {
		...
		// MyTown.instance.log.info("Checking condition: " +
		// StringUtils.join(conditionString, " "));
		boolean current;
		...
		for (int i = 0; i < conditionString.length; i += 4) {
			...
			if (StringUtils.tryParseBoolean(conditionString[i + 2])) {
				...
				throw new ConditionException(
						"The element with number " + ((i / 4) + 1) + " has an invalid type to be checked against!");
			}

			if (conditionString.length <= i + 3 || current && "OR".equals(conditionString[i + 3])
					|| !current && "AND".equals(conditionString[i + 3]))
				return current;
