package EmpiresMod.protection.Segment.Getter;

import java.lang.reflect.InvocationTargetException;
import java.util.ArrayList;
import java.util.List;

import EmpiresMod.exceptions.Protection.GetterException;
import EmpiresMod.protection.Segment.Caller.Caller;

/**
 * From a list of Callers it tries to get a value of type with all its getters
 * calling on the instance. OR It simply gets a constant available in the
 * constants. If method call fails with no parameters it's gonna try to add the
	...
	public Object invoke(Class<?> returnType, Object instance, Object... parameters) throws GetterException {
		...
		for (Caller caller : callers) {
			try {
				...
				throw new GetterException("Failed to access field/method " + caller.getName() + " in getter: " + name,
						iaex);
				...
				throw new GetterException(
						"Failed to get Integer type in getter: " + name + ". Found " + caller.getName(), numex);
			} catch (IndexOutOfBoundsException iobex) {
				throw new GetterException("Integer out of bounds in getter: " + name + ". Found " + caller.getName(),
						iobex);
			} catch (Exception ex) {
		...
		if (lastCaller.getValueType() != null && !lastCaller.getValueType().isAssignableFrom(lastInstance.getClass())) {
			throw new GetterException(
					"Failed to get " + lastCaller.getValueType().getSimpleName() + " type in getter: " + name);
		}
