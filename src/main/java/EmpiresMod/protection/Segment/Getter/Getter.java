	...
	public static class Serializer extends SerializerTemplate<Getter> {
		...
		public JsonElement serialize(Getter src, Type typeOfSrc, JsonSerializationContext context) {
			...
			if (src instanceof GetterConstant) {
				...
				return context.serialize(((GetterDynamic) src).callers, new TypeToken<List<Caller>>() {
				}.getType());
			}
			return null;
		}
		...
		@Override
		public Getter deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
			...
			if (json.isJsonPrimitive()) {
				...
				getter = new GetterDynamic((List<Caller>) context.deserialize(json, new TypeToken<List<Caller>>() {
				}.getType()));
			}
