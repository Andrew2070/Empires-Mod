...
import com.andrew2070.Empires.utils.ColorUtils;
...
import com.andrew2070.Empires.exceptions.PermissionCommandException;
...
public class PermCommands {
    ...
    protected static Group getGroupFromName(String name) {
        ...
        if(group == null) {
            throw new PermissionCommandException("Empires.cmd.err.group.notExist", LocalManager.get("Empires.format.group.short", name));
        }
        return group;
    }
    ...
    protected static UUID getUUIDFromUsername(String username) {
        ...
        if(uuid == null) {
            throw new PermissionCommandException("Empires.cmd.err.player.notExist", LocalManager.get("Empires.format.user.short", username));
        }
        return uuid;
    }
    ...
    public static class MyPermissionManagerCommands {
        ...
        public static CommandResponse groupListCommand(ICommandSender sender, List<String> args) {
            ...
            root.appendSibling(LocalManager.get("Empires.format.list.header", new ChatComponentFormatted("{9|GROUPS}")));

            for (Group group : getManager().groups) {
                ChatComponentText parents = new ChatComponentText("");
                for (Group parent : group.parents) {
                    IChatComponent parentComponent = LocalManager.get("Empires.format.group.parent", new ChatComponentText(parent.getName()));
                ...
                root.appendSibling(LocalManager.get("Empires.format.group.long", group.getName(), parents));
            }
        ...
        public static CommandResponse userListCommand(ICommandSender sender, List<String> args) {
            ...
            root.appendSibling(LocalManager.get("Empires.format.list.header", new ChatComponentFormatted("{9|USERS}")));

            for (User user : getManager().users) {
                root.appendSibling(LocalManager.get("Empires.format.user.long", user.lastPlayerName, user.group));
            }
