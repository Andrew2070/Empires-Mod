...
import com.EmpireMod.Empires.Handlers.WorldUtils;
...
/**
 * All utilities that are exclusively for EntityPlayerMP or EntityPlayer go
 * here.
 */
public class PlayerUtils {
	...
	/**
	 * Takes the amount of items specified. Returns false if player doesn't have
	 * the items necessary
	 */
	...
	public static boolean takeItemFromPlayer(EntityPlayer player, ItemStack itemStack, int amount) {
		...
		for (int i : slots) {
			if (player.inventory.mainInventory[i].stackSize >= amount) {
				...
				((EntityPlayerMP) player).playerNetServerHandler.sendPacket(new S2FPacketSetSlot(
						player.openContainer.windowId, slot.slotNumber, player.inventory.mainInventory[i]));
				return true;
			} else {
	...
	public static void giveItemToPlayer(EntityPlayer player, Item item, int amount, int meta) {
		...
		while (left > 0) {
			...
			if (i == -1) {
				// No matching ItemStacks that can be filled, look for empty
				// slots
				for (int j = 0; j < player.inventory.mainInventory.length; j++) {
			...
			if (i == -1) {
				...
				WorldUtils.dropAsEntity(player.getEntityWorld(), (int) player.posX, (int) player.posY,
						(int) player.posZ, itemStack);
			} else {
				// get the actual inventory Slot:
				Slot slot = player.openContainer.getSlotFromInventory(player.inventory, i);
				// send S2FPacketSetSlot to the player with the new / changed
				// stack (or null)
				((EntityPlayerMP) player).playerNetServerHandler.sendPacket(new S2FPacketSetSlot(
						player.openContainer.windowId, slot.slotNumber, player.inventory.mainInventory[i]));
			}
	...
	public static void giveItemStackToPlayer(EntityPlayer player, ItemStack itemStack) {
		...
		if (i != -1) {
			...
			// get the actual inventory Slot:
			Slot slot = player.openContainer.getSlotFromInventory(player.inventory, i);
			// send S2FPacketSetSlot to the player with the new / changed stack
			// (or null)
			((EntityPlayerMP) player).playerNetServerHandler.sendPacket(new S2FPacketSetSlot(
					player.openContainer.windowId, slot.slotNumber, player.inventory.mainInventory[i]));
		} else {
			WorldUtils.dropAsEntity(player.getEntityWorld(), (int) player.posX, (int) player.posY, (int) player.posZ,
					itemStack);
		}
	}
	...
	/**
	 * Teleports a player to (x, y, z) in dimension dim without creating any
	 * nether portals of sorts. Also preserves motion and potion effects even on
	 * cross dimensional teleports. Only the player teleports, leaving mounts
	 * behind.
	 * <p/>
	...
	/**
	 * Transfers a player to a new dimension preserving potion effects and
	 * motion.
	 */
