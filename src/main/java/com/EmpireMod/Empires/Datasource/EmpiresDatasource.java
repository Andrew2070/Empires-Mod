...
public class EmpiresDatasource extends DatasourceSQL {
	...
	public boolean loadAll() {
		return loadWorlds() && loadEmpires() && loadRanks() && loadBlocks() && loadCitizens() && loadPlots()
				&& /* loadNations() && */ loadEmpireFlags() && loadPlotFlags() && loadBlockWhitelists()
				&& loadEmpireInvites() && loadBlockOwners() && loadEmpireBanks() && loadRankPermissions()
				&& loadCitizensToEmpires() && /* loadEmpiresToNations() && */
	...
	protected boolean loadEmpires() {
		try {
			...
			while (rs.next()) {
				...
				empire.setSpawn(new Teleport(rs.getInt("spawnDim"), rs.getFloat("spawnX"), rs.getFloat("spawnY"),
						rs.getFloat("spawnZ"), rs.getFloat("cameraYaw"), rs.getFloat("cameraPitch")));
	...
	protected boolean loadBlocks() {
		try {
			...
			while (rs.next()) {
				...
				EmpireBlock block = new EmpireBlock(rs.getInt("dim"), rs.getInt("x"), rs.getInt("z"),
						rs.getBoolean("isFarClaim"), rs.getInt("pricePaid"), empire);
	...
	public boolean loadCitizens() { // was protected boolean
		try {
			...
			while (rs.next()) {
				Citizen res = new Citizen(UUID.fromString(rs.getString("uuid")), rs.getString("name"),
						rs.getLong("joined"), rs.getLong("lastOnline"));
	...
	protected boolean loadPlots() {
		try {
			...
			while (rs.next()) {
				Empire empire = getUniverse().empires.get(rs.getString("empireName"));
				Plot plot = new Plot(rs.getString("name"), empire, rs.getInt("dim"), rs.getInt("x1"), rs.getInt("y1"),
						rs.getInt("z1"), rs.getInt("x2"), rs.getInt("y2"), rs.getInt("z2"));
		...
		try {
			...
			while (rs.next()) {
				...
				try {
					...
					PreparedStatement removeFlag = prepare(
							"DELETE FROM " + prefix + "EmpireFlags WHERE empireName=? AND name=?", true);
				...
				try {
					...
					LOG.error(ExceptionUtils.getStackTrace(ex));
					PreparedStatement updateFlag = prepare(
							"UPDATE " + prefix + "EmpireFlags SET serializedValue=? WHERE empireName=? AND name=?",
							false);
		...
		try {
			...
			while (rs.next()) {
				...
				try {
					...
					LOG.error(ExceptionUtils.getStackTrace(ex));
					PreparedStatement removeFlag = prepare(
							"DELETE FROM " + prefix + "PlotFlags WHERE plotID=? AND name=?", true);
				...
				try {
					...
					LOG.error(ExceptionUtils.getStackTrace(ex));
					PreparedStatement updateFlag = prepare(
							"UPDATE " + prefix + "PlotFlags SET serializedValue=? WHERE plotID=? AND name=?", false);
					updateFlag.setString(1, flagType.serializeValue(flagType.defaultValue));
	...
	protected boolean loadBlockWhitelists() {
		try {
			...
			while (rs.next()) {
				...
				BlockWhitelist bw = new BlockWhitelist(rs.getInt("dim"), rs.getInt("x"), rs.getInt("y"), rs.getInt("z"),
						FlagType.valueOf(rs.getString("flagName")));
	...
	public boolean saveEmpire(Empire empire) {
		...
		try {
			if (getUniverse().empires.contains(empire)) { // Update
				PreparedStatement updateStatement = prepare("UPDATE " + prefix
						+ "Empires SET name=?, spawnDim=?, spawnX=?, spawnY=?, spawnZ=?, cameraYaw=?, cameraPitch=?, extraBlocks=?, maxPlots=?, extraFarClaims=?, currentPower=?, maxPower=? WHERE name=?",
						true);
				...
				updateStatement.setFloat(4, empire.getSpawn().getY());
				updateStatement.setFloat(5, empire.getSpawn().getZ());
				updateStatement.setFloat(6, empire.getSpawn().getYaw());
				updateStatement.setFloat(7, empire.getSpawn().getPitch());
				...
				// Need to move the Empire in the map from the old name to the
				// new
				if (empire.getOldName() != null) {
				...
				PreparedStatement insertStatement = prepare("INSERT INTO " + prefix
						+ "Empires (name, spawnDim, spawnX, spawnY, spawnZ, cameraYaw, cameraPitch, isAdminEmpire, extraBlocks, maxPlots, extraFarClaims) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
						true);
				insertStatement.setString(1, empire.getName());
				insertStatement.setInt(2, empire.getSpawn().getDim());
				insertStatement.setFloat(3, empire.getSpawn().getX());
				insertStatement.setFloat(4, empire.getSpawn().getY());
				insertStatement.setFloat(5, empire.getSpawn().getZ());
				insertStatement.setFloat(6, empire.getSpawn().getYaw());
				insertStatement.setFloat(7, empire.getSpawn().getPitch());
				insertStatement.setBoolean(8, empire instanceof AdminEmpire);
				insertStatement.setInt(9, empire.empireBlocksContainer.getExtraBlocks());
				insertStatement.setInt(10, empire.plotsContainer.getMaxPlots());
				insertStatement.setInt(11, empire.empireBlocksContainer.getExtraFarClaims());
	...
	public boolean saveBlock(EmpireBlock block) {
		...
		try {
			if (getUniverse().blocks.contains(block)) { // Update
				...
				PreparedStatement insertStatement = prepare(
						"INSERT INTO " + prefix
								+ "Blocks (dim, x, z, isFarClaim, pricePaid, empireName) VALUES (?, ?, ?, ?, ?, ?)",
						true);
	...
	public boolean saveRank(Rank rank) { // TODO Insert any new permissions to
											// the RankPermission table
		...
		try {
			if (getUniverse().ranks.contains(rank)) { // Update
				try {
					...
					PreparedStatement s = prepare(
							"UPDATE " + prefix + "Ranks SET type=?, name=? WHERE name=? AND empireName=?", true);
					...
					if (!rank.permissionsContainer.isEmpty()) {
						s = prepare("INSERT INTO " + prefix + "RankPermissions(node, rank, empireName) VALUES(?, ?, ?)",
								true);
				...
				try {
					...
					PreparedStatement insertRankStatement = prepare(
							"INSERT INTO " + prefix + "Ranks (name, empireName, type) VALUES(?, ?, ?)", true);
					...
					if (!rank.permissionsContainer.isEmpty()) {
						PreparedStatement insertRankPermStatement = prepare(
								"INSERT INTO " + prefix + "RankPermissions(node, rank, empireName) VALUES(?, ?, ?)",
								true);
						for (String perm : rank.permissionsContainer) {
	...
	public boolean saveRankPermission(Rank rank, String perm) {
		LOG.debug("Saving RankPermission {} for Rank {} in Empire {}", perm, rank.getName(),
				rank.getEmpire().getName());
		try {
			PreparedStatement s = prepare(
					"INSERT INTO " + prefix + "RankPermissions (node, rank, empireName) VALUES(?, ?, ?)", true);
			s.setString(1, perm);
			s.setString(2, rank.getName());
			s.setString(3, rank.getEmpire().getName());
			s.execute();
	...
	public boolean saveCitizen(Citizen citizen) {
		...
		try {
			if (getUniverse().citizens.contains(citizen.getUUID())) { // Update
				PreparedStatement updateStatement = prepare("UPDATE " + prefix
						+ "Citizens SET name=?, lastOnline=?, extraBlocks=?, power=?, lastPowerUpdateTime=?, isBanned=?, fakePlayer=? WHERE uuid=?",
						true);
				...
				PreparedStatement insertStatement = prepare("INSERT INTO " + prefix
						+ "Citizens (uuid, name, joined, lastOnline, extraBlocks, power, lastPowerUpdateTime, fakePlayer, isBanned) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)",
						true);
		...
		try {
			if (getUniverse().plots.contains(plot)) { // Update
				PreparedStatement statement = prepare(
						"UPDATE " + prefix + "Plots SET name=?, dim=?, x1=?, y1=?, z1=?, x2=?, y2=?, z2=? WHERE ID=?",
						true);
				...
				PreparedStatement insertStatement = prepare("INSERT INTO " + prefix
						+ "Plots (name, dim, x1, y1, z1, x2, y2, z2, empireName) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)",
						true);
	...
	public boolean saveFlag(Flag flag, Plot plot) {
		...
		try {
			if (plot.flagsContainer.contains(flag.flagType)) {
				// Update
				PreparedStatement s = prepare(
						"UPDATE " + prefix + "PlotFlags SET serializedValue=? WHERE plotID=? AND name=?", true);
				...
				// Insert
				PreparedStatement s = prepare(
						"INSERT INTO " + prefix + "PlotFlags(name, serializedValue, plotID) VALUES(?, ?, ?)", true);
	...
	public boolean saveFlag(Flag flag, Empire empire) {
		...
		try {
			if (empire.flagsContainer.contains(flag.flagType)) {
				// Update
				PreparedStatement updateStatement = prepare(
						"UPDATE " + prefix + "EmpireFlags SET serializedValue=? WHERE empireName=? AND name=?", true);
				...
				// Insert
				PreparedStatement insertStatement = prepare(
						"INSERT INTO " + prefix + "EmpireFlags(name,  serializedValue, empireName) VALUES(?, ?, ?)",
						true);
	...
	public boolean saveBlockWhitelist(BlockWhitelist bw, Empire empire) {
		try {
			if (!empire.blockWhitelistsContainer.contains(bw)) {
				PreparedStatement insertStatement = prepare("INSERT INTO " + prefix
						+ "BlockWhitelists(dim, x, y, z, flagName, empireName) VALUES(?, ?, ?, ?, ?, ?)", true);
	...
	public boolean saveSelectedEmpire(Citizen res, Empire empire) {
		try {
			if (res.empiresContainer.isSelectedEmpireSaved) {
				PreparedStatement statement = prepare(
						"UPDATE " + prefix + "SelectedEmpire SET empireName=? WHERE citizen=?", true);
				statement.setString(1, empire.getName());
				statement.setString(2, res.getUUID().toString());
				statement.executeUpdate();
			} else {
				PreparedStatement statement = prepare(
						"INSERT INTO " + prefix + "SelectedEmpire(citizen, empireName) VALUES(?, ?)", true);
	...
	public boolean saveEmpireInvite(Citizen res, Empire empire) {
		try {
			if (!res.empireInvitesContainer.contains(empire)) {
				PreparedStatement s = prepare(
						"INSERT INTO " + prefix + "EmpireInvites(citizen, empireName) VALUES(?, ?)", true);
	...
	public boolean saveEmpireBans(Citizen res, Empire empire) {
		try {
			if (!res.empireInvitesContainer.contains(empire)) {
				PreparedStatement s = prepare("INSERT INTO " + prefix + "EmpireBans(citizen, empireName) VALUES(?, ?)",
						true);
				s.setString(1, res.getUUID().toString());
				s.setString(2, empire.getName());
				s.executeUpdate();
			...
			LOG.error("Failed to save empire bans for citizen: {} for empire {}", res.getPlayerName(),
					empire.getName());
			LOG.error(ExceptionUtils.getStackTrace(e));
			return false;
		}
	...
	public boolean saveBlockOwner(Citizen res, int dim, int x, int y, int z) {
		try {
			PreparedStatement s = prepare(
					"INSERT INTO " + prefix + "BlockOwners(citizen, dim, x, y, z) VALUES(?, ?, ?, ?, ?)", false);
			s.setString(1, res.getUUID().toString());
			s.setInt(2, dim);
			s.setInt(3, x);
			s.setInt(4, y);
	...
	public boolean saveEmpireBank(Bank bank) {
		try {
			if (getUniverse().banks.contains(bank)) {
				PreparedStatement s = prepare(
						"UPDATE " + prefix + "EmpireBanks SET amount=?, daysNotPaid=? WHERE empireName=?", false);
	...
	public boolean linkCitizenToEmpire(Citizen res, Empire empire, Rank rank) {
		try {
			PreparedStatement s = prepare(
					"INSERT INTO " + prefix + "CitizensToEmpires (citizen, empire, rank) VALUES(?, ?, ?)", true);
			s.setString(1, res.getUUID().toString());
			s.setString(2, empire.getName());
			// You need rank since this method is the one that adds the citizen
			// to the empire and vice-versa
			s.setString(3, rank.getName());
			s.execute();
			...
			LOG.error("Failed to link Citizen {} ({}) with Empire {}", res.getPlayerName(), res.getUUID(),
					empire.getName());
			LOG.error(ExceptionUtils.getStackTrace(e));
			return false;
		}
	...
	public boolean unlinkCitizenFromEmpire(Citizen res, Empire empire) {
		try {
			PreparedStatement s = prepare(
					"DELETE FROM " + prefix + "CitizensToEmpires WHERE citizen = ? AND empire = ?", true);
			s.setString(1, res.getUUID().toString());
			s.setString(2, empire.getName());
			s.execute();
			...
			LOG.error("Failed to unlink Citizen {} ({}) with Empire {}", e, res.getPlayerName(), res.getUUID(),
					empire.getName());
			LOG.error(ExceptionUtils.getStackTrace(e));
			return false;
		}
	...
	public boolean updateCitizenToEmpireLink(Citizen res, Empire empire, Rank rank) {
		try {
			PreparedStatement s = prepare(
					"UPDATE " + prefix + "CitizensToEmpires SET rank = ? WHERE citizen = ? AND empire = ?", true);
			s.setString(1, rank.getName());
			...
			LOG.error("Failed to update link between Citizen {} ({}) with Empire {}", e, res.getPlayerName(),
					res.getUUID(), empire.getName());
			LOG.error(ExceptionUtils.getStackTrace(e));
			return false;
		}
		return true;
	...
	public boolean linkCitizenToPlot(Citizen res, Plot plot, boolean isOwner) {
		try {
			PreparedStatement s = prepare(
					"INSERT INTO " + prefix + "CitizensToPlots(citizen, plotID, isOwner) VALUES(?, ?, ?)", true);
			s.setString(1, res.getUUID().toString());
			s.setInt(2, plot.getDbID());
			...
			LOG.error("Failed to link {} to plot {} in empire {}", res.getPlayerName(), plot.getName(),
					plot.getEmpire().getName());
			LOG.error(ExceptionUtils.getStackTrace(e));
			return false;
		}
		return true;
	...
	public boolean unlinkCitizenFromPlot(Citizen res, Plot plot) {
		try {
			PreparedStatement s = prepare("DELETE FROM " + prefix + "CitizensToPlots WHERE citizen=? AND plotID=?",
					true);
			s.setString(1, res.getUUID().toString());
			s.setInt(2, plot.getDbID());
			s.executeUpdate();
			...
			LOG.error("Failed to unlink {} to plot {} in empire {}", res.getPlayerName(), plot.getName(),
					plot.getEmpire().getName());
			LOG.error(ExceptionUtils.getStackTrace(e));
			return false;
		}
	...
	public boolean updateCitizenToPlotLink(Citizen res, Plot plot, boolean isOwner) {
		try {
			PreparedStatement s = prepare(
					"UPDATE " + prefix + "CitizensToPlots SET isOwner=? WHERE citizen=? AND plotID=?", true);
			...
			LOG.error("Failed to update link {} to plot {} in empire {}", res.getPlayerName(), plot.getName(),
					plot.getEmpire().getName());
			LOG.error(ExceptionUtils.getStackTrace(e));
			return false;
		}
	...
	public boolean deleteBlock(EmpireBlock block) {
		try {
			...
			PreparedStatement deleteBlockStatement = prepare(
					"DELETE FROM " + prefix + "Blocks WHERE dim=? AND x=? AND z=?", true);
	...
	public boolean deleteRank(Rank rank) {
		try {
			...
			PreparedStatement deleteRankStatement = prepare(
					"DELETE FROM " + prefix + "Ranks WHERE name=? AND empireName=?", true);
	...
	public boolean deleteEmpireInvite(Citizen res, Empire empire, boolean response) {
		try {
			PreparedStatement s = prepare("DELETE FROM " + prefix + "EmpireInvites WHERE citizen=? AND empireName=?",
					true);
			s.setString(1, res.getUUID().toString());
			s.setString(2, empire.getName());
			s.executeUpdate();
	...
	public boolean deleteFlag(Flag flag, Empire empire) {
		try {
			PreparedStatement deleteFlagStatement = prepare(
					"DELETE FROM " + prefix + "EmpireFlags WHERE name=? AND empireName=?", true);
	...
	public boolean deleteFlag(Flag flag, Plot plot) {
		try {
			PreparedStatement deleteFlagStatement = prepare(
					"DELETE FROM " + prefix + "PlotFlags WHERE name=? AND plotID=?", true);
			deleteFlagStatement.setString(1, flag.flagType.toString());
	...
	public boolean deleteRankPermission(Rank rank, String perm) {
		try {
			PreparedStatement s = prepare(
					"DELETE FROM " + prefix + "RankPermissions WHERE node = ? AND rank = ? AND empireName = ?", true);
			s.setString(1, perm);
			s.setString(2, rank.getName());
			s.setString(3, rank.getEmpire().getName());
			s.execute();
			...
			LOG.error("Failed to add permission ({}) to Rank ({}) in Empire ({})", perm, rank.getName(),
					rank.getEmpire().getName());
			LOG.error(ExceptionUtils.getStackTrace(e));
			return false;
		}
		...
		for (Empire empire : getUniverse().empires) {
			for (FlagType type : FlagType.values()) {
				if (type.isEmpirePerm && !empire.flagsContainer.contains(type)) {
					...
					LOG.info("Flag {} in empire {} got created because of the settings.", type.name.toLowerCase(),
							empire.getName());
				}
			}
		}
		...
		for (Plot plot : getUniverse().plots) {
			for (FlagType type : FlagType.values()) {
				if (type.isPlotPerm && !plot.flagsContainer.contains(type)) {
					...
					LOG.info("Flag {} in plot {} of empire {} got created because of the settings.",
							type.name.toLowerCase(), plot.getName(), plot.getEmpire().getName());
				}
			}
		}
