...
public class EmpiresUniverse { // TODO Allow migrating between different
								// Datasources
	...
	// public final Map<String, Alliance> alliances = new HashMap<String,
	// Alliance>();
	...
	/**
	 * Creates and returns a new Empire with basic entities saved to db, or null
	...
	private void configureEmpire(Empire empire, Citizen creator) {
		...
		/*
		 * for (int dim : EmpiresUniverse.instance.getWorldsList()) { if
		 * (MinecraftServer.getServer().worldServerForDimension(dim) == null) {
		 * deleteWorld(dim); } }
		 */

		// Setting spawn before saving
		empire.setSpawn(new Teleport(creator.getPlayer().dimension, (float) creator.getPlayer().posX,
				(float) creator.getPlayer().posY, (float) creator.getPlayer().posZ, creator.getPlayer().cameraYaw,
				creator.getPlayer().cameraPitch));
		...
		if (EmpiresUniverse.instance.blocks.contains(dim, chunkX, chunkZ)) {
			...
			// throw new
			// EmpiresCommandException("Empires.cmd.err.claim.already");
		}
		...
		if (!(empire instanceof AdminEmpire)) {
			...
			if (!getDatasource().linkCitizenToEmpire(creator, empire, empire.ranksContainer.getLeaderRank())) {
				Empires.instance.LOG.error("Problem linking citizen {} to empire {}", creator.getPlayerName(),
						empire.getName());
			}
	...
	public Citizen getOrMakeCitizen(UUID uuid, String playerName, boolean isFakePlayer) {
		...
		if (res == null) {
			...
			if (res != null && !getDatasource().saveCitizen(res)) { // Only save
																	// if a new
																	// Citizen
				return null;
			}
	 ...
	 * alliances.put(alliance.getName(), alliance); return true; }
	 */
