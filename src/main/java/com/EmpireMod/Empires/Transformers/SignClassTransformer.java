package com.EmpireMod.Empires.Transformers;
...
/**
 * Do a simple modification on {@link net.minecraft.tileentity.TileEntitySign}
 * to add a custom field and two static invocations. <br/>
 * The final code would be:
 * 
 * <pre>
 * <code>
 *     public class TileEntitySign extends TileEntity{
 *         public NBTTagCompound Empires;
 *         // ... original fields
 *
...
public class SignClassTransformer implements IClassTransformer {
	 ...
	 */
	private static Field EmpiresDataField;

	/**
	 * Get the cached field or initialize it if it's not initialized yet
	 * 
	 * @param sign
	 *            A instance of {@link net.minecraft.tileentity.TileEntitySign}.
	 *            It's an {@link Object} arg to simplify the ASM code
	 *            generation.
	 * @return The cached field
	 * @throws RuntimeException
	 *             if the transformer failed
	 */
	private static Field getEmpiresDataField(Object sign) {
		...
		return EmpiresDataField;
	}

	/**
	 * Read the custom field stored in a patched
	 * {@link net.minecraft.tileentity.TileEntitySign}.
	 * 
	 * @param sign
	 *            A instance of {@link net.minecraft.tileentity.TileEntitySign}.
	 ...
	 */
	@Nullable
	public static NBTTagCompound getEmpiresDataValue(Object sign) {
		try {
			return (NBTTagCompound) getEmpiresDataField(sign).get(sign);
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
	 ...
	 *            The custom data.
	 */
	public static void setEmpiresDataValue(Object sign, @Nullable NBTTagCompound modData) {
		try {
			getEmpiresDataField(sign).set(sign, modData);
	...
	// ------- CORE-MOD PART ------- //
	// This part patches the TileEntitySign to add the custom field provide
	// save/load support

	/**
	 * Method executed before
	 * {@link net.minecraft.tileentity.TileEntitySign#writeToNBT(NBTTagCompound)}
	 * returns
	 * 
	 * @param sign
	 *            A instance of {@link net.minecraft.tileentity.TileEntitySign}.
	 ...
	 *            generation.
	 * @param tagCompound
	 *            The NBT tag that is being written
	 */
	@SuppressWarnings("unused")
	public static void writeToNBT(Object sign, NBTTagCompound tagCompound) {
		NBTTagCompound modData = getEmpiresDataValue(sign);
		if (modData != null)
	 ...
	 *            generation.
	 * @param tagCompound
	 *            The NBT tag that is being read
	 */
	@SuppressWarnings("unused")
	public static void readFromNBT(Object sign, NBTTagCompound tagCompound) {
		...
		setEmpiresDataValue(sign, modData);
	}

	/**
	 * Adds an static invocation to a method on this class, the local method
	 * must return {@code void} and accept the arguments {@link Object} and
	 * {@link NBTTagCompound}
	 */
	private class SignGeneratorAdapter extends GeneratorAdapter {
		 ...
		 * 
		 * @param mv
		 *            The method that is being visited
		 * @param access
		 *            The method access flag
		 * @param name
		 *            The method name
		 * @param desc
		 *            The method description (arguments and return type)
		 * @param localMethodName
		 *            The static method on {@link SignClassTransformer} that
		 *            will be executed before the visited method returns
		 */
		...
		/**
		 * Visit all instructions and add an static invocation before the method
		 * returns
		 * 
		 * @param opcode
		...
		public void visitInsn(int opcode) {
			...
			if (opcode == Opcodes.RETURN) {
				// Add:
				// Empires.classtransformers.SignClassTransformer.<localMethodName>(this,
				// arg1);
				// before the return statement
				super.visitVarInsn(Opcodes.ALOAD, 0);
				super.visitVarInsn(Opcodes.ALOAD, 1);
				super.visitMethodInsn(Opcodes.INVOKESTATIC, "com/EmpireMod/Empires/Utilities/SignClassTransformer", localMethodName,
	 ...
	 * 
	 * @param name
	 *            Obfuscated name of the class that is being loaded
	 * @param srgName
	 *            SRG name of the class that is being loaded
	 * @param bytes
	...
	public byte[] transform(String name, String srgName, byte[] bytes) {
		...
		if ("net.minecraft.tileentity.TileEntitySign".equals(srgName)) {
			...
			// This will iterate over everything on the class and allow us to
			// modify what we want
			ClassVisitor visitor = new ClassVisitor(Opcodes.ASM4, writer) {
				/**
				 * Modify the writeToNBT and readFromNBT methods to save/load
				 * our custom data
				 * 
				 * @param access
				 *            The current method's access flag
				 * @param name
				 *            The current method's name
				 * @param desc
				 *            The current method's description (arguments and
				 *            return type)
				 * @param signature
				 *            The current method's signature
				 * @param exceptions
				 *            The exceptions that can be thrown by the current
				 *            method
		...
		// If it was the sign, return the modified class, else return the
		// original class
		return bytes;
	}
}