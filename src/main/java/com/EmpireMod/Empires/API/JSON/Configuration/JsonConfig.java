...
/**
 * An abstract class for all JSON configs. Methods are usually overrided but
 * called inside their overrided method.
 */
public abstract class JsonConfig<T, L extends List<T>> {
	...
	public void create(L initialItems) {
		try {
			...
			writer.close();
			Empires.instance.LOG.info("Created new " + name + " file successfully!");
		} catch (IOException ex) {
			Empires.instance.LOG.error(ExceptionUtils.getStackTrace(ex));
			Empires.instance.LOG.error("Failed to create " + name + " file!");
		}
	}
	...
	public void write(L items) {
		try {
			...
			writer.close();
			Empires.instance.LOG.info("Updated the " + name + " file successfully!");
		} catch (IOException ex) {
			Empires.instance.LOG.error(ExceptionUtils.getStackTrace(ex));
			Empires.instance.LOG.error("Failed to update " + name + " file!");
		}
	}
	...
	public L read() {
		...
		try {
			...
			reader.close();
			Empires.instance.LOG.info("Loaded " + name + " successfully!");
		} catch (IOException ex) {
			Empires.instance.LOG.error(ExceptionUtils.getStackTrace(ex));
			Empires.instance.LOG.error("Failed to read from " + name + " file!");
		}
