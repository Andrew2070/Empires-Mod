...
public class RanksConfig extends JsonConfig<Rank, Rank.Container> {
	...
	public RanksConfig(String path) {
		...
		this.gson = new GsonBuilder().registerTypeAdapter(Rank.class, new Rank.Serializer()).setPrettyPrinting()
				.create();
	}
		...
		for (Rank.Type type : Rank.Type.values()) {
			if (type.unique) {
				...
				if (rankOfType.size() == 0) {
					...
					isValid = false;
					Empires.instance.LOG.error("Unique type of Rank was found multiple times in " + name
							+ ". Setting all aside from the first to type regular.");
