package com.EmpireMod.Empires.API.JSON;
...
/**
 * Default flags on empire creation config file.
 */
    ...
    public boolean validate(List<Wrapper> items) {
        ...
        for (Iterator<Wrapper> it = items.iterator(); it.hasNext(); ) {
            ...
            if (item.flagName == null) {
                Empires.instance.LOG.error("Found a type of flag that does not exist. Removing...");
                it.remove();
                isValid = false;
                continue;
            }
            if (!item.getFlagType().type.isAssignableFrom(item.defaultState.getClass())) {
                Empires.instance.LOG.error("The default value for the flag is of invalid type for flag " + item.flagName.toString() + "! Needed " + item.getFlagType().type.getSimpleName() + " Removing...");
                it.remove();
                isValid = false;
            }
        }
        ...
        for (FlagType type : FlagType.values()) {
            ...
            if (!ok) {
                items.add(new Wrapper(type.name, type.defaultValue, type.configurable));
                Empires.instance.LOG.error("Flag config is missing (or is an invalid entry) {} flag! Adding with default settings...", type.name);
                isValid = false;
            }
        }
        return isValid;
