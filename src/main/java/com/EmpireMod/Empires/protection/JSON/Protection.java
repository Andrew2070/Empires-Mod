package com.EmpireMod.Empires.protection.JSON;
...
public class Protection {
	...
	public static class Serializer extends SerializerTemplate<Protection> {
		...
		public JsonElement serialize(Protection protection, Type typeOfSrc, JsonSerializationContext context) {
			...
			json.add("segments", context.serialize(protection.segments, new TypeToken<List<Segment>>() {
			}.getType()));
		...
		@Override
		public Protection deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
			...
			if (jsonObject.has("segments")) {
				protection.segments.addAll(
						(List<Segment>) context.deserialize(jsonObject.get("segments"), new TypeToken<List<Segment>>() {
						}.getType()));
			}
		...
		private static boolean isModLoaded(String modid, String name, String version) {
			...
			for (ModContainer mod : Loader.instance().getModList()) {
				if (mod.getModId().equals(modid) && (name.length() == 0 || mod.getName().equals(name))
						&& mod.getVersion().startsWith(version)) {
					return true;
				}
			}
			return false;
