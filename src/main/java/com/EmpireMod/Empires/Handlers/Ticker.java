...
public class Ticker {
	...
	public void onTickEvent(TickEvent.WorldTickEvent ev) {
		...
		if ((Config.instance.costEmpireUpkeep.get() > 0 || Config.instance.costAdditionalUpkeep.get() > 0)
				&& ev.phase == TickEvent.Phase.START) {
			if (ticked) {
				if (lastCalendarDay != -1 && Calendar.getInstance().get(Calendar.DAY_OF_YEAR) != lastCalendarDay) {
					for (int i = 0; i < EmpiresUniverse.instance.empires.size(); i++) {
						...
						if (!(empire instanceof AdminEmpire)) {
							...
							if (empire.bank.getDaysNotPaid() == Config.instance.upkeepEmpireDeletionDays.get()
									&& Config.instance.upkeepEmpireDeletionDays.get() > 0) {
								Empires.instance.LOG.info(
										"Empire {} has been deleted because it didn't pay upkeep for {} days.",
										empire.getName(), Config.instance.upkeepEmpireDeletionDays.get());
		...
		for (int i = 0; i < allPlayers.size(); i++) {
			...
			if (player != null) {
				...
				if (player instanceof EntityPlayer) {
					...
					// Check To See if Player Has A Citizen Profile, If Not Then
					// Make One:
	...
	public void onPlayerDeath(PlayerEvent.PlayerRespawnEvent ev) {
		...
		Empires.instance.datasource.saveCitizen(res);
		ChatManager.send(ev.player, "Empires.notification.ciz.powerLostOnDeath", Config.instance.PowerPerDeath.get(),
				res.getPower());
	...
	public void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent ev) {
		...
		if (res != null) {
			...
			Empires.instance.LOG.error("[Player Login] Didn't create citizen for player {} ({})",
					ev.player.getCommandSenderName(), ev.player.getPersistentID());
