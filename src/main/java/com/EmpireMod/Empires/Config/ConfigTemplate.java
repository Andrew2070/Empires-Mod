...
public abstract class ConfigTemplate {
        ...
        for (Field field : getClass().getDeclaredFields()) {
            if (field.getType().isAssignableFrom(ConfigProperty.class)) {
                try {
                    ...
                    Empires.instance.LOG.error("Failed to access " + field.getName() + " while binding to config " + config.getConfigFile().getName());
    ...
    private void setProperty(ConfigProperty property, Property forgeProp) {
        try {
            ...
            Empires.instance.LOG.error("Config value of " + property.name + " in category " + property.category + " was not of the proper type!");
            Empires.instance.LOG.error(ExceptionUtils.getStackTrace(ex));
            throw ex;
        }
    }
