...
public class CommandsNeutral extends CommandsEMP {
	@Command(name = "info", permission = "Empires.cmd.outsider.info", parentName = "Empires.cmd", syntax = "/empire info [empire]", completionKeys = {
	...
	public static CommandResponse infoCommand(ICommandSender sender, List<String> args) {
		...
		for (Empire empire : empires) {
			IChatComponent header = LocalizationManager.get("Empires.format.list.header",
					new ChatComponentFormatted("{9|%s}", empire.getName()));
			ChatManager.send(sender, "Empires.format.empire.long", header, empire.citizensMap.size(),
					empire.empireBlocksContainer.size(), empire.getMaxBlocks(), empire.getPower(), empire.getMaxPower(),
					empire.citizensMap, empire.ranksContainer);
		}
		return CommandResponse.DONE;
	}

	@Command(name = "citizen", permission = "Empires.cmd.outsider.res", parentName = "Empires.cmd", syntax = "/empire citizen <citizen>", completionKeys = {
			"citizenCompletion" }, alias = { "ciz", "Citizen" }, console = true)
	public static CommandResponse resCommand(ICommandSender sender, List<String> args) {
		if (args.size() < 1) {
			...
			ChatManager.send(sender, "Empires.format.citizen.long", header, res1.empiresContainer,
					Formatter.formatDate(res1.getJoinDate()), Formatter.formatDate(res1.getLastOnline()),
					res1.getExtraBlocks(), res1.getPower(), res1.getMaxPower());
			return CommandResponse.DONE;
		}
		...
		ChatManager.send(sender, "Empires.format.citizen.long", header, res.empiresContainer,
				Formatter.formatDate(res.getJoinDate()), Formatter.formatDate(res.getLastOnline()),
				res.getExtraBlocks(), res.getPower(), res.getMaxPower());
		return CommandResponse.DONE;
	}

	@Command(name = "list", permission = "Empires.cmd.outsider.list", parentName = "Empires.cmd", syntax = "/empire list [page]", console = true)
	...
	@Command(name = "create", permission = "Empires.cmd.outsider.new", parentName = "Empires.cmd", syntax = "/empire create <name>")
	public static CommandResponse newEmpireCommand(ICommandSender sender, List<String> args) {
		...
		Citizen res = EmpiresUniverse.instance.getOrMakeCitizen(sender); // Attempt
																			// to
																			// get
																			// or
																			// make
		...
		for (int x = ((int) player.posX >> 4)
				- Config.instance.distanceBetweenEmpires.get(); x <= ((int) player.posX >> 4)
						+ Config.instance.distanceBetweenEmpires.get(); x++) {
			for (int z = ((int) player.posZ >> 4)
					- Config.instance.distanceBetweenEmpires.get(); z <= ((int) player.posZ >> 4)
				...
				if (nearbyEmpire != null && !nearbyEmpire.flagsContainer.getValue(FlagType.NEARBY)) {
					throw new EmpiresCommandException("Empires.cmd.err.new.tooClose", nearbyEmpire,
							Config.instance.distanceBetweenEmpires.get());
				}
			}
		}

		// Need to insert an If statement that blocks the use of spaces in
		// empire names ie: Republic of LALA has 2 spaces, hence unsearchable.
		...
		Empire empire = getUniverse().newEmpire(args.get(0), res); // Attempt to
																	// create
																	// the
																	// Empire
	...
	@Command(name = "map", permission = "Empires.cmd.outsider.map", parentName = "Empires.cmd", syntax = "/empire map [on|off]")
	...
	@Command(name = "accept", permission = "Empires.cmd.outsider.accept", parentName = "Empires.cmd", syntax = "/empire accept [empire]", completionKeys = {
			"empireCompletion" })
	public static CommandResponse acceptCommand(ICommandSender sender, List<String> args) {
		...
		if (args.size() == 0) {
			...
			// Basically true only if player specifies a empire that is not in
			// its invites
	...
	@Command(name = "refuse", permission = "Empires.cmd.outsider.refuse", parentName = "Empires.cmd", syntax = "/empire refuse [empire]", completionKeys = {
			"empireCompletion" })
	...
	@Command(name = "help", permission = "Empires.cmd.outsider.help", parentName = "Empires.cmd", syntax = "/empire help <command>", alias = {
			"?", "h" }, console = true)
	...
	@Command(name = "syntax", permission = "Empires.cmd.outsider.syntax", parentName = "Empires.cmd", syntax = "/empire syntax <command>", console = true)
	...
	@Command(name = "invites", permission = "Empires.cmd.outsider.invites", parentName = "Empires.cmd", syntax = "/empire invites")
	...
	@Command(name = "prices", permission = "Empires.cmd.outsider.prices", parentName = "Empires.cmd", syntax = "/empire prices")
	public static CommandResponse pricesCommand(ICommandSender sender, List<String> args) {
		...
		IChatComponent header = LocalizationManager.get("Empires.format.list.header",
				new ChatComponentFormatted("{9|PRICES}"));
		ChatManager.send(sender, "Empires.notification.prices", header,
