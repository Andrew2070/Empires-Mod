package com.EmpireMod.Empires.commands.Officer;
...
public class CommandsOfficer extends CommandsEMP {
	...
	@Command(name = "setspawn", permission = "Empires.cmd.officer.setspawn", parentName = "Empires.cmd", syntax = "/empire setspawn")
	public static CommandResponse setSpawnCommand(ICommandSender sender, List<String> args) {
		...
		empire.getSpawn().setDim(player.dimension)
	...
	@Command(name = "claim", permission = "Empires.cmd.officer.claim", parentName = "Empires.cmd", syntax = "/empire claim [range]")
	public static CommandResponse claimCommand(ICommandSender sender, List<String> args) {
		...
		if (args.size() < 1) {
			...
			for (int x = player.chunkCoordX - Config.instance.distanceBetweenEmpires.get(); x <= player.chunkCoordX
					+ Config.instance.distanceBetweenEmpires.get(); x++) {
				for (int z = player.chunkCoordZ - Config.instance.distanceBetweenEmpires.get(); z <= player.chunkCoordZ
						+ Config.instance.distanceBetweenEmpires.get(); z++) {
					Empire nearbyEmpire = EmpireUtils.getEmpireAtPosition(player.dimension, x, z);
					if (nearbyEmpire != null && nearbyEmpire != empire
							&& !nearbyEmpire.flagsContainer.getValue(FlagType.NEARBY)) {
						throw new EmpiresCommandException("Empires.cmd.err.claim.tooClose", nearbyEmpire,
								Config.instance.distanceBetweenEmpires.get());
					}
				}
			}
			...
			int price = (isFarClaim ? Config.instance.costAmountClaimFar.get() : Config.instance.costAmountClaim.get())
					+ Config.instance.costAdditionClaim.get() * empire.empireBlocksContainer.size();
			...
			for (Iterator<ChunkPos> it = chunks.iterator(); it.hasNext();) {
				...
				for (int x = chunk.getX() - Config.instance.distanceBetweenEmpires.get(); x <= chunk.getX()
						+ Config.instance.distanceBetweenEmpires.get(); x++) {
					for (int z = chunk.getZ() - Config.instance.distanceBetweenEmpires.get(); z <= chunk.getZ()
							+ Config.instance.distanceBetweenEmpires.get(); z++) {
						Empire nearbyEmpire = EmpireUtils.getEmpireAtPosition(player.dimension, x, z);
						if (nearbyEmpire != null && nearbyEmpire != empire
								&& !nearbyEmpire.flagsContainer.getValue(FlagType.NEARBY)) {
			...
			makeBankPayment(player, empire,
					(isFarClaim
							...
							+ MathUtils.sumFromNtoM(empire.empireBlocksContainer.size(),
									empire.empireBlocksContainer.size() + chunks.size() - 1)
									* Config.instance.costAdditionClaim.get());

			for (ChunkPos chunk : chunks) {
				int price = (isFarClaim ? Config.instance.costAmountClaimFar.get()
						: Config.instance.costAmountClaim.get())
						+ Config.instance.costAdditionClaim.get() * empire.empireBlocksContainer.size();
				EmpireBlock block = getUniverse().newBlock(player.dimension, chunk.getX(), chunk.getZ(), isFarClaim,
						price, empire);
				// Only one of the block will be a farClaim, rest will be normal
				// claim
				isFarClaim = false;
				getDatasource().saveBlock(block);
				ChatManager.send(sender, "Empires.notification.block.added", block.getX() * 16, block.getZ() * 16,
	...
	@Command(name = "unclaim", permission = "Empires.cmd.officer.unclaim", parentName = "Empires.cmd", syntax = "/empire unclaim")
	public static CommandResponse unclaimCommand(ICommandSender sender, List<String> args) {
		...
		if (!checkNearby(block.getDim(), block.getX(), block.getZ(), empire)
				&& empire.empireBlocksContainer.size() <= 1) {
	...
	@Command(name = "overclaim", permission = "Empires.cmd.officer.overclaim", parentName = "Empires.cmd", syntax = "/empire overclaim")
	public static CommandResponse overclaimCommand(ICommandSender sender, List<String> args) {
		...
		if (block.isPointIn(empire.getSpawn().getDim(), empire.getSpawn().getX(), empire.getSpawn().getZ())) {
			...
			empire.notifyEveryone(
					getLocal().getLocalization("Empires.notification.empire.overclaimed.deleted", empire, res));
		}

		getDatasource().deleteBlock(block);
		ChatManager.send(sender, "Empires.notification.block.overclaimed", block.getX() << 4, block.getZ() << 4,
				block.getX() << 4 + 15, block.getZ() << 4 + 15, empire);
		return CommandResponse.DONE;
	}

	@Command(name = "invite", permission = "Empires.cmd.officer.invite", parentName = "Empires.cmd", syntax = "/empire invite <citizen>", completionKeys = {
			"citizenCompletion" })
	...
	@Command(name = "ban", permission = "Empires.cmd.officer.ban", parentName = "Empires.cmd", syntax = "/empire ban <citizen>", completionKeys = {
			"citizenCompletion" })
	...
	@Command(name = "unban", permission = "Empires.cmd.officer.unban", parentName = "Empires.cmd", syntax = "/empire unban <citizen>", completionKeys = {
			"citizenCompletion" })
	...
	@Command(name = "set", permission = "Empires.cmd.officer.perm.set", parentName = "Empires.cmd.everyone.perm", syntax = "/empire perm set <flag> <value>", completionKeys = "flagCompletion")
	...
	@Command(name = "toggle", permission = "Empires.cmd.officer.perm.toggle", parentName = "Empires.cmd.everyone.perm", syntax = "/empire perm toggle <flag>", completionKeys = "flagCompletion")
	...
	@Command(name = "whitelist", permission = "Empires.cmd.officer.perm.whitelist", parentName = "Empires.cmd.everyone.perm", syntax = "/empire perm whitelist")
	...
	@Command(name = "promote", permission = "Empires.cmd.officer.promote", parentName = "Empires.cmd", syntax = "/empire promote <citizen> <rank>", completionKeys = {
			"citizenCompletion", "rankCompletion" })
	...
	public static class ModifyRanks {
		...
		@Command(name = "add", permission = "Empires.cmd.officer.ranks.add", parentName = "Empires.cmd.everyone.ranks", syntax = "/empire ranks add <name> [templateRank]", completionKeys = {
				"-", "ranksCompletion" })
		public static CommandResponse ranksAddCommand(ICommandSender sender, List<String> args) {
			...
			if (empire.ranksContainer.contains(args.get(0))) {
				throw new EmpiresCommandException("Empires.cmd.err.ranks.add.already",
						empire.ranksContainer.get(args.get(0)));
			}

			Rank rank = new Rank(args.get(0), empire, Rank.Type.REGULAR);
			if (args.size() == 2) {
		...
		@Command(name = "remove", permission = "Empires.cmd.officer.ranks.remove", parentName = "Empires.cmd.everyone.ranks", syntax = "/empire ranks remove <rank>", completionKeys = {
				"rankCompletion" })
		...
		@Command(name = "set", permission = "Empires.cmd.officer.ranks.set", parentName = "Empires.cmd.everyone.ranks", syntax = "/empire ranks set <rank> <type>", completionKeys = {
				"rankCompletion" })
		...
		@Command(name = "add", permission = "Empires.cmd.officer.ranks.perm.add", parentName = "Empires.cmd.officer.ranks.perm", syntax = "/empire ranks perm add <rank> <perm>", completionKeys = {
				"rankCompletion" })
		...
		@Command(name = "remove", permission = "Empires.cmd.officer.ranks.perm.remove", parentName = "Empires.cmd.officer.ranks.perm", syntax = "/empire ranks perm remove <rank> <perm>", completionKeys = {
				"rankCompletion" })
		...
		@Command(name = "reset", permission = "Empires.cmd.officer.ranks.reset", parentName = "Empires.cmd.everyone.ranks", syntax = "/empire ranks reset")
	...
	@Command(name = "perm", permission = "Empires.cmd.officer.ranks.perm", parentName = "Empires.cmd.everyone.ranks", syntax = "/empire ranks perm <command>")
	public static CommandResponse ranksPermCommand(ICommandSender sender, List<String> args) {
		return CommandResponse.SEND_HELP_MESSAGE;
	}

	@Command(name = "list", permission = "Empires.cmd.officer.ranks.perm.list", parentName = "Empires.cmd.officer.ranks.perm", syntax = "/empire ranks perm list [rank]")
	...
	@Command(name = "abdicate", permission = "Empires.cmd.leader.pass", parentName = "Empires.cmd", syntax = "/empire abdicate <citizen>", completionKeys = {
			"citizenCompletion" })
	...
	public static class Plots {
		@Command(name = "limit", permission = "Empires.cmd.officer.plot.limit", parentName = "Empires.cmd.everyone.plot", syntax = "/empire plot limit <command>")
		public static CommandResponse plotLimitCommand(ICommandSender sender, List<String> args) {
			return CommandResponse.SEND_HELP_MESSAGE;
		}

		@Command(name = "show", permission = "Empires.cmd.officer.plot.limit.show", parentName = "Empires.cmd.officer.plot.limit", syntax = "/empire plot limit show")
		...
		@Command(name = "set", permission = "Empires.cmd.officer.plot.limit.set", parentName = "Empires.cmd.officer.plot.limit", syntax = "/empire plot limit set <limit>")
	...
	@Command(name = "kick", permission = "Empires.cmd.officer.kick", parentName = "Empires.cmd", syntax = "/empire kick <citizen>", completionKeys = {
			"citizenCompletion" })
	...
	@Command(name = "delete", permission = "Empires.cmd.leader.leave.delete", parentName = "Empires.cmd.everyone.leave", syntax = "/empire leave delete")
	...
	@Command(name = "rename", permission = "Empires.cmd.officer.rename", parentName = "Empires.cmd", syntax = "/empire rename <name>")
	...
	@Command(name = "withdraw", permission = "Empires.cmd.officer.bank.withdraw", parentName = "Empires.cmd.everyone.bank", syntax = "/empire bank withdraw <amount>")
	public static CommandResponse bankWithdrawCommand(ICommandSender sender, List<String> args) {
		...
		if (empire.bank.getAmount() < amount) {
			throw new EmpiresCommandException("Empires.cmd.err.bank.withdraw",
					EconomyProxy.getCurrency(empire.bank.getAmount()));
		}
	...
	@Command(name = "chunkload", permission = "Empires.cmd.officer.claim.chunkload", parentName = "Empires.cmd.officer.claim", syntax = "/empire claim chunkload")
	...
	@Command(name = "chunkunload", permission = "Empires.cmd.officer.claim.chunkunload", parentName = "Empires.cmd.officer.claim", syntax = "/empire claim chunkunload")
	...
	@Command(name = "all", permission = "Empires.cmd.officer.claim.chunkload.all", parentName = "Empires.cmd.officer.claim.chunkload", syntax = "/empire claim chunkload all")
	...
	@Command(name = "all", permission = "Empires.cmd.officer.claim.chunkunload.all", parentName = "Empires.cmd.officer.claim.chunkunload", syntax = "/empire claim chunkunload all")
